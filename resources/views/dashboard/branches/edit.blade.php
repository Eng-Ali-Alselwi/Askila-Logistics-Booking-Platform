@extends('dashboard.layout.admin', ['title' => t('Update Branch')])
...
@section('content')
    ...
    <x-dashboard.outer-card :title="t('Update Branch')">
        ...
        <div class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800">
            <form action="{{ route('dashboard.branches.update', $branch) }}" method="POST" class="p-5 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select id="country" name="country"
                            ...
                            <option value="SA" {{ old('country', $branch->country) == 'SA' ? 'selected' : '' }}>{{ t('Saudi Arabia') }}</option>
                            <option value="SD" {{ old('country', $branch->country) == 'SD' ? 'selected' : '' }}>{{ t('Sudan') }}</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select id="is_active" name="is_active"
                            ...
                            <option value="1" {{ old('is_active', $branch->is_active ? '1' : '0') == '1' ? 'selected' : '' }}>{{ t('Active') }}</option>
                            <option value="0" {{ old('is_active', $branch->is_active ? '1' : '0') == '0' ? 'selected' : '' }}>{{ t('Inactive') }}</option>
                        </select>
                ...
                <div class="flex items-center gap-3 pt-2">
                    ...
                    <a href="{{ route('dashboard.branches.show', $branch) }}" class="px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700">{{ t('Cancel') }}</a>
                </div>
            </form>
        </div>
